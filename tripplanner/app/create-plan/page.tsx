...
import { countriesData } from '@/public/data/countries'; // Adjust the path as necessary
...
const CreatePlan = ({ initialPlan }: any) => {
    ...
    const fetchUserPlans = async () => {
      try {
        ...
        if (plans.length > 0) {
          ...
          router.push(`/edit-plan/${plans[0].id}`);
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (userPlans.length > 0) {
      ...
      router.push(`/edit-plan/${userPlans[0].id}`);
    }
  }, [userPlans, router]);
...
const calculateTotalCost = (itinerary: ItineraryData): number => {
  ...
  const handleSavePlan = async () => {
    ...
    const endpoint = plan.id
      ? `http://localhost:3001/api/itinerary/plans/${plan.id}`
      : 'http://localhost:3001/api/itinerary/plans';
