...
const Navbar: React.FC<NavbarProps> = ({ className, onNavigate }) => {
  ...
  const confirmBecomeSeller = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:3001/api/users/${userId}/updateRole`, {
        method: 'POST',
        headers: {
  ...
  return (
    <nav className={`bg-gradient-to-r from-white to-blue-200 p-3 ${className}`}>
      <div className="mx-auto flex items-center justify-between">
        {/* Logo */}
        <div className="ml-12 flex items-center flex-grow-0">
        ...
        {/* Desktop Menu */}
        <ul className="hidden lg:flex xl:flex  space-x-8 text-lg font-medium mx-auto">
          <li>
          ...
          {!isLoggedIn ? (
            <>
              <a  href="/register" onClick={(e) => { e.preventDefault(); onNavigate('/register'); }}>
                <button className="ml-4 px-4 py-2 bg-gradient-to-r  from-green-500 to-green-600 text-white font-semibold shadow-md hover:from-green-600 hover:to-green-700 transition-transform transform hover:scale-105 flex items-center">
                  <FaUserPlus className="mr-2" /> Register
                </button>
              </a>
              <a href="/login" onClick={(e) => { e.preventDefault(); onNavigate('/login'); }}>
                <button className="ml-4 px-4 py-2 bg-gradient-to-r from-blue-500 to-blue-600 text-white font-semibold shadow-md hover:from-blue-600 hover:to-blue-700 transition-transform transform hover:scale-105 flex items-center">
            ...
            <div className="relative">
              <button onClick={toggleProfileMenu} className="ml-4 mr-2 flex items-center px-6 py-2 bg-gradient-to-r from-blue-400 to-blue-600 text-white font-semibold shadow-md transition-transform transform hover:scale-105">
        ...
        <div className="flex lg:hidden xl:hidden items-center">
          <button onClick={toggleMobileMenu} className="text-blue-800 focus:outline-none">
            <span className="block w-8 h-1 bg-blue-800 mb-2"> </span>
            <span className="block w-8 h-1 bg-blue-800 mb-2"></span>
            <span className="block w-8 h-1 bg-blue-800"></span>
          </button>
        </div>
      </div>
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <ul className="md:hidden  flex flex-col space-y-4 mt-4 text-lg font-medium text-blue-800">
          <li>
          ...
          {!isLoggedIn ? (
            <>
              <li>
                <a href="/register" onClick={(e) => { e.preventDefault(); onNavigate('/register'); }} className="hover:border-b-4 mx-4 hover:border-green-500 transition-colors flex items-center">
                  <FaUserPlus className="mr-2" /> Register
                </a>
              </li>
              <li>
                <a href="/login" onClick={(e) => { e.preventDefault(); onNavigate('/login'); }} className="hover:border-b-4 mx-4 hover:border-blue-500 transition-colors flex items-center">
                  <FaSignInAlt className="mr-2" /> Login
                </a>
              </li>
            </>
